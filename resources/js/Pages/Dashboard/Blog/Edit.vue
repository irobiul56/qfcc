...
const form = useForm({
    ...
    category_id: props.blog?.category_id?.toString() || '',
    image: null,
    _method: 'PUT'
});

// Image preview
const imagePreview = ref(props.blog?.image ? `/storage/${props.blog.image}` : null);
...
// Submit form
const updateblog = () => {
    form.post(route("blogpost.update", props.blog.id), {
        forceFormData: true,
        preserveScroll: true,
        onSuccess: () => {
        ...
        onError: (errors) => {
            ElMessage.error("Failed to update blog. Please check the form.");
        }
    });
};
</script>
...
<template>
    ...
    <UserLayout>
        <template #header>
            <h2 class="text-xl font-semibold leading-tight text-gray-800">Edit Blog</h2>
        </template>
            ...
            <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
                <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-900">
                        <h1 class="mb-6 text-2xl font-bold text-gray-800">Edit blog</h1>
